...
from cycling_overlay.config import DEFAULT_FOCUS_FILE, DEFAULT_OPACITY, parse_args
...
def test_saved_settings_used_when_no_cli_override(tmp_path, monkeypatch):
    ...
    settings_file.write_text(
        json.dumps({"min_cadence": 65, "imperial": True, "opacity": 0.5}),
        encoding="utf-8",
    )
...
def test_cli_flag_takes_precedence_over_saved_settings(tmp_path, monkeypatch):
    settings_file = tmp_path / "settings.json"
    settings_file.write_text(json.dumps({"min_power": 100, "opacity": 0.6}), encoding="utf-8")
    monkeypatch.setattr(settings_module, "SETTINGS_FILE", settings_file)

    cfg = parse_args(["--min-power", "250"])

    assert cfg.min_power == 250  # CLI wins
    assert cfg.opacity == 0.6  # untouched saved value still applies
